...
/**
 * Analisa diff do git de forma abrangente
 */
export async function analyzeDiff(staged: boolean = true): Promise<DiffAnalysis> {
  const baseArgs = staged ? ['diff', '--cached'] : ['diff'];
  
  try {
    // Comando separado para status dos arquivos
    const { stdout: statusOutput } = await execaCommand('git', [...baseArgs, '--name-status']);
    
    // Se não há arquivos staged/unstaged via diff, verificar arquivos não rastreados
    if (!statusOutput.trim() && !staged) {
      const { stdout: untrackedOutput } = await execaCommand('git', ['ls-files', '--others', '--exclude-standard']);
      if (untrackedOutput.trim()) {
        // Tratar arquivos não rastreados como novos
        const untrackedFiles = untrackedOutput.trim().split('\n').filter(Boolean);
        const files = await Promise.all(
          untrackedFiles.map(async (file): Promise<FileChange> => ({
            file,
            status: 'A',
            insertions: 0, // Não calculamos linhas para arquivos não rastreados
            deletions: 0,
            hunks: [],
            isBinary: false
          }))
    ...
    const { stdout: statsOutput } = await execaCommand('git', [...baseArgs, '--numstat']);
 ...
 */
async function parseFileChanges(statsOutput: string, statusOutput: string, staged: boolean): Promise<FileChange[]> {
  ...
  const statsMap = new Map<string, { insertions: number; deletions: number; isBinary: boolean }>();
  ...
  for (const statusLine of statusLines) {
    ...
    const stats = statsMap.get(filePath) || { insertions: 0, deletions: 0, isBinary: false };
 ...
 */
async function getFileHunks(filePath: string, staged: boolean): Promise<DiffHunk[]> {
  const args = staged 
  ...
  for (const line of lines) {
    ...
    if (currentHunk && (line.startsWith(' ') || line.startsWith('+') || line.startsWith('-'))) {
      ...
      currentHunk.lines.push({
        type,
        content,
        lineNumber: type === '+' ? currentHunk.newStart + currentHunk.lines.filter(l => l.type === '+' || l.type === ' ').length
                  : type === '-' ? currentHunk.oldStart + currentHunk.lines.filter(l => l.type === '-' || l.type === ' ').length
                  : undefined
      });
    }
  }
 ...
 */
export function generateFileSummaries(analysis: DiffAnalysis): Array<{ file: string; bullets: string[] }> {
